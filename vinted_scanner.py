...
def send_slack_message(item_brand, item_title, item_price, item_url, item_image):
    ...
    # Format message content
    message_lines = [f"*Brand: {item_brand}*", f"*{item_title}*", f"🏷️ {item_price}", f"🔗 {item_url}"]
    if item_image:
...
def send_telegram_message(item_brand, item_title, item_price, item_url, item_image):
    ...
    # Format message content
    message_lines = [f"<b>Brand: {item_brand}</b>", f"<b>{item_title}</b>", f"🏷️ {item_price}", f"🔗 {item_url}"]
    if item_image:
        message_lines.append(f"📷 {item_image}")
    message = "\n".join(message_lines)
...
def main():
    ...
    for params in Config.queries:
        ...
        if data:
            ...
            for item in data["items"]:
                item_id = str(item["id"])
                item_brand = item.get("brand_title", "N/A")
